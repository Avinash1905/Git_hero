/**
 * CommitSearchPage
 * Master workbench for querying commit logs and Pickaxe code search.
 */

import { CommitSearchEngine } from '../features/search/CommitSearchEngine.js';

export function renderCommitSearchPage() {
  const engine = new CommitSearchEngine();
  const html = engine.renderHtml();
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 border border-primary/30 text-primary text-xs font-terminal-label font-bold uppercase tracking-wider mb-2">
            ...
            <span>Historical Query Engine</span>
          </div>
          <h1 class="text-headline-md font-headline-md text-on-surface font-bold">
            Git Commit Search & Pickaxe
          </h1>
          <p class="text-on-surface-variant text-sm font-terminal-code">
            Filter commits by log message regex, author identities, or search exact code strings via git log -S
          </p>
        </div>
      </div>
